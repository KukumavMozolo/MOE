...
class GaussianProcess(GaussianProcessInterface):
    ...
    def compute_variance_of_points(self, points_to_sample):
        ts = numpy.linspace(self.low, self.high, self.n)
        var = 0
        for i in ts:
            points_to_sample[:,self.idx] = i
            var+= self._compute_variance_of_points(points_to_sample)
        return var *1.0/self.n
    ...
    def compute_mean_of_points(self, points_to_sample):
        ...
        for i in ts:
            points_to_sample[:,self.idx] = i
            mean += self._compute_mean_of_points(points_to_sample)
        return mean * 1.0/self.n

    def compute_grad_mean_of_points(self, points_to_sample, num_derivatives=-1):
        ts = numpy.linspace(self.low, self.high, self.n)
        num_derivatives = self._clamp_num_derivatives(points_to_sample.shape[0], num_derivatives)

        mean_grad_mu = numpy.empty((num_derivatives, self.dim))
        for i in ts:
            points_to_sample[:,self.idx] = i
            mean_grad_mu += self._compute_grad_mean_of_points(points_to_sample, num_derivatives)
        return mean_grad_mu * 1.0/self.n
    ...
    def _compute_grad_variance_of_points_per_point(self, points_to_sample, var_of_grad):
        ...
        for i in ts:
            points_to_sample[:,self.idx] = i
            grad_var+= self.__compute_grad_variance_of_points_per_point(points_to_sample, var_of_grad)
        return grad_var *1.0/self.n
